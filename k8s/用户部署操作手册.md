...
- 密码通过 Helm `--set-string` 注入，不要把真实密码写入 `k8s/middlewares/clickhouse.values.yaml`
...
部署前请确认已导出 `VALKEY_PASSWORD`。密码通过 Helm `--set-string` 注入，不要把真实密码写入 `k8s/middlewares/valkey.values.yaml`。
...
当前 values 已按单节点配置：`replicaCount: 1`，并关闭 `config.cluster.enabled`。部署前请确认已导出 `QDRANT_API_KEY`。API Key 通过 Helm `--set-string` 注入，不要把真实值写入 `k8s/middlewares/qdrant.values.yaml`。
